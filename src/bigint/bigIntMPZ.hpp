...
public:
  ...
  bool operator == (const unsigned int& ui) const;
  bool operator == (const unsigned long int& ui) const;
  bool operator == (const signed int& si) const;
  bool operator == (const signed long int& di) const;
  ...
  bool operator < (const unsigned long int& ui) const;
  bool operator < (const signed int& si) const;
  bool operator < (const signed long int& si) const;
  ...
  const BigIntMpz& operator += (const unsigned int& ui);
  const BigIntMpz& operator += (const unsigned long int& ui);
  const BigIntMpz& operator += (const signed int& si);
  const BigIntMpz& operator += (const signed long int& si);
  ...
  virtual std::ostream& write(std::ostream& os) const {return os << m_BigIntMpz;}
  /** Function to return a string */
  virtual const std::string toString() const {return m_BigIntMpz.get_str();}
  ...
  inline const mpz_class getBigInt() const {return m_BigIntMpz;}
...
* from GMP lib - should not be used by the user, only internally */
  ...
  int compMpzVsUi(const unsigned long int& ui) const;
  int compMpzVsSi(const signed long int& si) const;
  int compMpzVsD(const double& d) const;
};
...
inline bool testIfPrime(const BigIntMpz& a, const int& reps = 50) {return a.testIfPrime(reps);}
